...
function topbar(title, back = false) {
  return `<header class="topbar compact">${back ? `<button class="back-btn" data-page="${state.page === "detail" ? "category" : "cart"}">${icons.back}<span>返回</span></button>` : `<div><div class="eyebrow">社区便利 · 店员配送</div><div class="brand">乐享<span>便利店</span></div></div>`}<strong>${title}</strong></header>`;
}
...
function homePage() {
  return `<section class="screen"><div class="home-hero-surface"><header class="topbar home-topbar"><div class="brand-row"><div><div class="brand home-brand">乐享<span>便利店</span></div><div class="eyebrow">欢迎回来，今天也要好好生活</div><div class="location">${icons.pin} 赤水本地 · 邻里超市</div></div></div>${searchBox()}</header><div class="home-hero-copy"><div class="hero-kicker">赤水竹瀑 · 清爽邻里</div><h1>赤水的鲜味<br/>送到家门口</h1><p>店员配送，仅限本小区<br/>满35元免配送费</p><button class="hero-action" data-page="category">去逛逛 ${icons.arrow}</button></div><div class="service-strip"><div class="service-left">${icons.truck}<span>店员配送 · 本小区内</span></div><div class="service-right">满35元免配送费</div></div></div><div class="content"><section class="section"><div class="section-head"><h2 class="section-title">常买分类</h2><button class="section-link" data-page="category">全部分类 ${icons.arrow}</button></div><div class="category-grid">${categories.map(([emoji, name]) => `<button class="category-item" data-category="${name}"><span class="category-icon">${emoji}</span><span>${name}</span></button>`).join("")}</div></section><section class="section"><div class="section-head"><h2 class="section-title">精选推荐</h2><button class="section-link" data-page="category">更多 ${icons.arrow}</button></div><div class="product-grid">${products.slice(0, 4).map(productCard).join("")}</div></section></div>${nav("home")}</section>`;
}
