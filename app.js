const products = [
  ...
  { id: 7, name: "可口可乐", meta: "330ml · 罐装", price: 3.0, emoji: "🥤", tone: "red", category: "酒水饮料" },
...
const icons = {
  ...
  edit: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m4 16-.7 4.7L8 20l11-11-4-4L4 16Z"/><path d="m13 6 4 4"/></svg>'
};
...
function productArt(product, extra = "") { return `<div class="product-art ${product.tone || ""} ${extra}">${product.emoji}</div>`; }
...
function nav(current) {
  ...
  return `<nav class="bottom-nav">${items.map(([page, icon, label]) => `<button class="nav-item ${current === page ? "active" : ""}" data-page="${page}">${icon}<span>${label}</span>${page === "cart" && cartCount() ? `<b class="nav-badge">${cartCount()}</b>` : ""}</button>`).join("")}</nav>`;
}

function topbar(title, back = false) {
  return `<header class="topbar compact">${back ? `<button class="back-btn" data-page="${state.page === "detail" ? "category" : "cart"}">${icons.back}<span>返回</span></button>` : `<div><div class="eyebrow">社区便利 · 店员配送</div><div class="brand">家门口<span>便利店</span></div></div>`}<strong>${title}</strong><div class="mini-actions"><button class="icon-btn">•••</button><button class="icon-btn">◉</button></div></header>`;
}

function homePage() {
  return `<section class="screen"><header class="topbar"><div class="brand-row"><div><div class="eyebrow">欢迎回来，今天也要好好生活</div><div class="location">${icons.pin} 绿城 · 幸福里小区⌄</div></div><div class="mini-actions"><button class="icon-btn">•••</button><button class="icon-btn">◉</button></div></div><label class="search">${icons.search}<input placeholder="搜一搜你想买的商品" /><button data-search>搜索</button></label></header><div class="content"><div class="hero"><div class="hero-copy"><div class="hero-kicker">新鲜 · 便捷 · 邻里好生活</div><h1>家门口的日常所需<br/>现在就能送到家</h1><p>店员配送，仅限本小区<br/>满35元免配送费</p><button class="hero-action" data-page="category">去逛逛 ${icons.arrow}</button></div></div><div class="service-strip"><div class="service-left">${icons.truck}<span>店员配送</span></div><div class="service-right">满35元免配送费</div></div><section class="section"><div class="section-head"><h2 class="section-title">常买分类</h2><button class="section-link" data-page="category">全部分类 ${icons.arrow}</button></div><div class="category-grid">${categories.map(([emoji, name]) => `<button class="category-item" data-category="${name}"><span class="category-icon">${emoji}</span><span>${name}</span></button>`).join("")}</div></section><section class="section"><div class="section-head"><h2 class="section-title">精选推荐</h2><button class="section-link" data-page="category">更多 ${icons.arrow}</button></div><div class="product-grid">${products.slice(0, 4).map(productCard).join("")}</div></section></div>${nav("home")}</section>`;
}
...
function categoryPage() {
  ...
  return `<section class="screen">${topbar("分类")}
    <div class="content"><label class="search" style="margin-top:0">${icons.search}<input placeholder="搜索商品" /><button data-search>搜索</button></label><div class="category-layout section"><aside class="category-rail">${categories.slice(0, 9).map(([, name]) => `<button class="rail-item ${state.category === name ? "active" : ""}" data-category="${name}">${name}</button>`).join("")}</aside><div class="category-products">${visible.map(listProduct).join("")}</div></div></div>${nav("category")}</section>`;
}

function listProduct(product) {
  return `<article class="list-product"><button class="open-product" data-product="${product.id}">${productArt(product)}</button><div><button class="open-product product-name" data-product="${product.id}">${product.name}</button><div class="product-meta">${product.meta}</div><div class="product-foot"><span class="price"><small>¥</small>${product.price.toFixed(2)}</span><span class="product-meta">有货</span></div></div><button class="add-btn" data-add="${product.id}">+</button></article>`;
}

function detailPage() {
  const p = state.selectedProduct;
  return `<section class="screen">${topbar("", true)}<div class="content"><div class="detail-photo ${p.tone}">${p.emoji}<span class="counter">1 / 4</span></div><h1 class="detail-title">${p.name} <small style="font-size:12px;color:var(--muted);font-weight:500">${p.meta.split("·")[0]}</small></h1><div class="detail-sub">脆甜多汁｜新鲜采摘｜今日可送</div><div class="detail-price">${money(p.price)}</div><div class="detail-section"><div class="detail-label">选择规格</div><div class="option-row"><button class="option">${p.meta.split("·")[0].trim()}</button><div class="qty-control"><button data-detail-minus>−</button><strong>1</strong><button data-detail-plus>＋</button></div></div></div><div class="detail-note">${icons.truck}<span>店员配送，仅限本小区配送</span></div><div class="detail-note"><span>✓</span><span>满35元免配送费，未满35元配送费5元</span></div></div><div class="detail-actions"><button class="secondary-btn" data-add="${p.id}">加入购物车</button><button class="primary-btn" data-page="cart">立即购买</button></div>${nav("")}</section>`;
}
